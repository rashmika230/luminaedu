...
import GeminiQA from './components/GeminiQA.tsx';
import Login from './components/Login.tsx';
import Timetable from './components/Timetable.tsx';
import Evaluation from './components/Evaluation.tsx';
import AdminManageCourses from './components/AdminManageCourses.tsx';
import AdminManageUsers from './components/AdminManageUsers.tsx';
import CourseContentManager from './components/CourseContentManager.tsx';
import LiveSessionManager from './components/LiveSessionManager.tsx';
import { User, AppRoute, UserRole, Course } from './types.ts';
...
const App: React.FC = () => {
          ...
          <div className="p-8 md:p-12 animate-in fade-in">
             <div className="max-w-4xl mx-auto bg-white rounded-[2.5rem] border border-slate-100 shadow-xl p-10">
                ...
                <div className="mt-12 pt-8 border-t border-slate-100 flex justify-end">
                   <button className="bg-slate-900 text-white px-8 py-3 rounded-xl font-bold hover:bg-indigo-600 transition-all shadow-lg">
                      Save Changes
                   </button>
                </div>
             </div>
      ...
      default:
        return (
          <div className="flex flex-col items-center justify-center h-full min-h-[500px] text-slate-400">
              ...
              <button onClick={() => setCurrentRoute(AppRoute.HOME)} className="mt-8 text-amber-600 font-black uppercase tracking-widest text-xs hover:underline">
  ...
  return (
    <div className="flex min-h-screen bg-slate-50">
      <Sidebar currentRoute={currentRoute} setRoute={setCurrentRoute} onLogout={handleLogout} userRole={user?.role} />
      <main className="flex-1 ml-16 md:ml-64 transition-all pb-24">
        ...
        <footer className="fixed bottom-0 right-0 left-16 md:left-64 bg-white/50 backdrop-blur-sm px-8 py-3 border-t border-slate-100 flex justify-between items-center z-10">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
            &copy; {new Date().getFullYear()} Developed by K.A.V.Rashmika.
          </p>
